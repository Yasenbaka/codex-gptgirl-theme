...
#    .\gptgirl.ps1 watch          前台运行自动守护（可 Ctrl+C 退出）
# ============================================================
param(
  [Parameter(Position = 0)]
  [ValidateSet('start', 'inject', 'check', 'remove', 'build', 'probe', 'doctor', 'install', 'uninstall', 'watch', 'help')]
  [string]$Action = 'help',
...
switch ($Action) {
  ...
  'start' {
    ...
    if (Test-DebugPort) {
      ...
      try { Start-Process -FilePath $exe -ArgumentList "--remote-debugging-port=$Port" }
